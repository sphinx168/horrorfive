...
- `GATES` — the puzzle "logins" the player must solve to advance chapters (Yahoo Mail login, the board's admin restore form, a search box). Each gate defines `fields` with an `answer` list checked via fuzzy `match()`/`norm()` (handles full-width/half-width punctuation, spacing, case — because answers are meant to be transcribed by ear from a co-op voice call, even though this build is single-player).
